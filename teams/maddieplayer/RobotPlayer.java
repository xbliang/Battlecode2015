...
public class RobotPlayer {
	public static void run(RobotController rc) {
        ...
        if (rc.getType() == 			RobotType.AEROSPACELAB) {
        	...
        	MapLocation avgAlliedTowerLoc = myself.getAverageTowerLocation(myself.myTeam);
        	MapLocation avgEnemyTowerLoc = myself.getAverageTowerLocation(myself.theirTeam);
        	
        	try {
        		rc.broadcast(0, avgAlliedTowerLoc.x);
        		rc.broadcast(1, avgAlliedTowerLoc.y);
        		rc.broadcast(2, avgEnemyTowerLoc.x);
        		rc.broadcast(3, avgEnemyTowerLoc.y);
        	} catch (Exception e) {
        		e.printStackTrace();
        	}
